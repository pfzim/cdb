<?php
  ...
  FROM [".TMAO_DB_NAME."].[dbo].[TBL_CLIENT_INFO] WHERE CLIENTTYPE = 0");
	...
	while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC))
	{
		...
		$db->put(rpv("INSERT INTO @computers (`name`, `ao_ptnupdtime`, `ao_script_ptn`, `ao_as_pstime`) VALUES (!, !, #, !) ON DUPLICATE KEY UPDATE `ao_ptnupdtime` = !, `ao_script_ptn` = #, `ao_as_pstime` = !, `flags` = (`flags` & ~0x0010)", $row['COMP_NAME'], $row['PTNUPDTIME'], $row['SCRIPT_PTN'], $row['AS_PSTIME'], $row['PTNUPDTIME'], $row['SCRIPT_PTN'], $row['AS_PSTIME']));
		$i++;
	}

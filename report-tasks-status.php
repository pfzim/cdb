...
EOT;
	...
	$html .= '<tr><td>Устаревшая БД антивируса</td><td>'.$result[0]['p_tmao'].' (ТТ: '.$result[0]['p_tmao_tt'].')</td><td>'.$result[0]['o_tmao'].'</td><th>'.TASKS_LIMIT_TMAO_GUP.'</th></tr>';
	$html .= '<tr><td>Не зашифрован ноутбук</td><td>'.$result[0]['p_tmee'].'</td><td>'.$result[0]['o_tmee'].'</td><th>∞</th></tr>';
	$html .= '<tr><td>Не обновляется пароль LAPS</td><td>'.$result[0]['p_laps'].'</td><td>'.$result[0]['o_laps'].'</td><th>'.TASKS_LIMIT_LAPS.'</th></tr>';
	$html .= '<tr><td>Агент SCCM не активен</td><td>'.$result[0]['p_sccm'].'</td><td>'.$result[0]['o_sccm'].'</td><th>'.TASKS_LIMIT_SCCM.'</th></tr>';
	$html .= '<tr><td>Имя ПК не соответствует стандарту именования</td><td>'.$result[0]['p_name'].'</td><td>'.$result[0]['o_name'].'</td><th>'.TASKS_LIMIT_RENAME.'</th></tr>';
	$html .= '<tr><td>Давно не устанавливались обновления</td><td>'.$result[0]['p_wsus'].' (ТТ: '.$result[0]['p_wsus_tt'].')</td><td>'.$result[0]['o_wsus'].'</td><th>'.TASKS_LIMIT_WSUS_GUP.'</th></tr>';
	$html .= '<tr><td>Не установлена квота на ПЯ</td><td>'.$result[0]['p_mbxq'].'</td><td>'.$result[0]['o_mbxq'].'</td><th>'.TASKS_LIMIT_MBX.'</th></tr>';
	$html .= '<tr><td>Устаревшая операционная система</td><td>'.$result[0]['p_os'].'</td><td>'.$result[0]['o_os'].'</td><th>'.TASKS_LIMIT_OS.'</th></tr>';
	$html .= '</table>';

	...
	*/
	...
	while($line !== FALSE)
	{
		...
		if(!empty($line))
		{
			...
			{
				if($db->put(rpv("INSERT INTO @mac (`pid`, `name`, `mac`, `ip`, `port`, `first`, `date`, `flags`, `vlan`) VALUES (#, !, !, !, !, NOW(), NOW(), #, ".(intval($row[5]) ?? "DEFAULT").")",
					$pid,
					$row[1],  // name
					$mac,
					$row[2],  // ip
			...
			{
				$row_id = $result[0][0];
				$db->put(rpv("UPDATE @mac SET `pid` = #,`name` = !, `ip` = !, `port` = !, `vlan` = ".(intval($row[5]) ?? "DEFAULT").", `first` = IFNULL(`first`, NOW()), `date` = NOW(), `flags` = ((`flags` & ~0x0002) | #) WHERE `id` = # LIMIT 1",
					$pid,
					$row[1],  // name
					$row[2],  // ip
					$row[4],  // port

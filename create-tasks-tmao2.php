<?php
	...
	if($db->select_assoc_ex($result, rpv("SELECT * FROM @computers WHERE (`flags` & (0x01 | 0x04 | 0x08)) = 0 AND `name` regexp '^(([[:digit:]]{4}-[nN])|([Pp][Cc]-))[[:digit:]]+$' AND `ao_script_ptn` < (SELECT MAX(`ao_script_ptn`) FROM c_computers) - 200")))
	{
		foreach($result as &$row)
		{
			...
			$answer = @file_get_contents('http://helpdesk.contoso.com/ExtAlert.aspx/?Source=cdb&Action=new&Type=tmao&Host='.urlencode($row['name']).'&Message='.urlencode("Выявлена проблема с TMAO\r\nПК: ".$row['name']."\r\nВерсия антивирусной базы: ".$row['ao_script_ptn']));
			if($answer !== FALSE)
			{
	...
	if($db->select_assoc_ex($result, rpv("SELECT * FROM @computers WHERE (`flags` & 0x08) AND `name` regexp '^(([[:digit:]]{4}-[nN])|([Pp][Cc]-))[[:digit:]]+$' AND (`ao_script_ptn` >= (SELECT MAX(`ao_script_ptn`) FROM c_computers) OR (`flags` & (0x01 | 0x04)))")))
	{
		foreach($result as &$row)
		{
			$answer = @file_get_contents('http://helpdesk.contoso.com/ExtAlert.aspx/?Source=cdb&Action=resolved&Type=tmao&Id='.urlencode($row['ao_operid']).'&Num='.urlencode($row['ao_opernum']).'&Host='.urlencode($row['name']).'&Message='.urlencode("Заявка более не актуальна"));
			if($answer !== FALSE)
			{
				$xml = @simplexml_load_string($answer);
				if($xml !== FALSE)

<?php
	// Report detected vulnerabilities (servers)

	/**
		\file
		\brief Формирование отчёта по обнаруженным уязвимостям - Серверы TOP 100.
		
		Отчёт формируется по уязвимостям загруженным из папки Nessus с идентификатором равным NESSUS_SERVERS_FOLDER_ID
	*/
...
<head>
	...
	<body>
	<h1>Отчёт по обнаруженным уязвимостям (серверы)</h1>
EOT;
	...
	$html .= $table;
	$html .= '<br /><small><a href="'.CDB_URL.'/cdb.php?action=report-vuln-top-servers">Сформировать отчёт заново</a></small>';
	$html .= '</body>';

	echo 'Opened: '.$i."\r\n";

	if(php_mailer(REPORT_VULNS_MAIL_TO, CDB_TITLE.': Detected vulnerabilities - Servers', $html, 'You client does not support HTML'))
	{
		echo 'Send mail: OK';
	}
	else
